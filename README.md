...
ЯДРО (ДВИЖОК) БРАУЗЕРА -  программа, преобразующая содержимое веб-страниц (файлы HTML, XML, цифровые изображения и т.д.)
   ...
   Api - интерфейс, через который мы можем общаться с сервером (точка касания)
   ```
   ## ЧТО ТАКОЕ ENDPOINTS
   ```
   Endpoint - адрес, куда полетит запрос серверу.
   ```
   ## URL (URI, URL, URN)
   ```
   ...
   ```
   ## ИДЕМПОТЕНТНЫЕ И БЕЗОПАСНЫЕ HTTP ЗАПРОСЫ
   ```
   ...
   ```
   - Идентификация - процедура распознавания субъекта по его идентификатору (пр. логин - это идентификатор, ты вводишь логин, система тебя идентифицирует)
   - Аутентификация - процедура проверки подлинности (пр. проверка подлинности пользователя путем сравнения введенного им пароля с паролем, сохрааненным в базе данных)
   - Авторизация - предоставление определенному пользователю прав на выполнение определенных действий (пр. после идентификации и аутентификации система предоставляет пользователю читать письма в его почтовом ящике )
   ...
   ## DNS, ОКТАТЫ
   ```
   DNS - протокол прикладного уровня 
   ```
   ## OSI
